...
public class ArticleDao {
	...
	public List<Article> findAllArticlesForTrade(Connection connection, PageInfo pageInfo) {
		...
		String query = "SELECT " + "    * " + "FROM " + "    ( " + "        SELECT " + "            ROWNUM AS RNUM, "
				+ "            NO, " + "            USER_NO, " + "            TYPE, " + "            TITLE, "
				+ "            CONTENT, " + "            READ_COUNT, " + "            VISABLE, "
				+ "            POST_DATE, " + "            EDITED, " + "            EDIT_DATE " + "        FROM "
				+ "            ( " + "                SELECT " + "                    ROWNUM AS RNUM, "
				+ "                    NO, " + "                    USER_NO, " + "                    TYPE, "
				+ "                    TITLE, " + "                    CONTENT, " + "                    READ_COUNT, "
				+ "                    VISABLE, " + "                    POST_DATE, " + "                    EDITED, "
				+ "                    EDIT_DATE " + "                FROM " + "                    ARTICLE "
				+ "                WHERE " + "                    TYPE IN ( '거래' ) " + "                ORDER BY "
				+ "                    NO DESC " + "            ) " + "    ) " + "WHERE RNUM BETWEEN ? AND ?";
	...
	public int getMostRecentlyArticleNoByMemberNo(Connection connection, int no) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String query = "SELECT " + "    MAX(NO) " + "FROM " + "    ( " + "        SELECT " + "            NO "
				+ "        FROM " + "            ARTICLE " + "        WHERE" + "            USER_NO = ?" + "    )";
	...
	public int saveTradeArticle(Connection connection, TradeArticle trart) {
		int result = 0;
		PreparedStatement pstmt = null;
		String query = "INSERT INTO TRADE_ARTICLE ( " + "    ARTICLE_NO, " + "    CLOTH_NO, " + "    PRICE, "
				+ "    TRADE_ENDED, " + "    FREE, " + "    TRADE_METHOD, " + "    LOCATION " + ") VALUES ( "
				+ "    ?, " + "    ?, " + "    ?,  " + "    DEFAULT, " + "    ?, " + "    ?, " + "    ? " + ")";
	...
	public int UpdateArticleForTrade(Connection connection, Article article) {
		int result1 = 0;
		PreparedStatement pstmt = null;
...
// 230216 5교시 댓글 조회
	...
	public List<Reply> getRepliesByNoForCommunity(Connection connection, int no) {
		...
		String query = "SELECT R.NO, " + "R.ARTICLE_NO, " + "R.CONTENT, " + "M.NICKNAME, " + "R.COMMENT_DATE, "
				+ "R.EDIT_DATE " + "FROM REPLY R " + "JOIN MEMBER M ON(R.ID_NO = M.NO) "
				+ "WHERE R.VISABLE='Y' AND ARTICLE_NO=? " + "ORDER BY R.NO DESC";
	...
	public int UpdateTradeArticle(Connection connection, TradeArticle trart) {
		int result2 = 0;
		PreparedStatement pstmt = null;
		String query = "UPDATE TRADE_ARTICLE "
				+ "SET PRICE = ?, "
				+ "    FREE = ?, "
				+ "    TRADE_METHOD = ?, "
				+ "    LOCATION = ? "
		...
		try {
			...
			pstmt.setInt(1, trart.getPrice());
			pstmt.setString(2, trart.getFree());
			pstmt.setString(3, trart.getTradeMethod());
			pstmt.setString(4, trart.getLocation());
			pstmt.setInt(5, trart.getNo());
